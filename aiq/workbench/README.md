...
├── public/config.json         placeholder config for local builds; the deploy step OVERWRITES this file
├── src/
│   ├── main.tsx               boot: /config.json → Cognito PKCE + real runtime API (no unauthenticated mode)
...
## No unauthenticated mode (security mandate, 2026-09-19)

Nothing renders without a Cognito session. Every route — including `/`, deep links such as `/p/<job>` and `/lab`, and any
query string — redirects to Cognito Managed Login immediately and returns the visitor to the route they asked for after
sign-in. The former fixtures/"mock mode" (`"mock": true`, `?mock=1`) was removed from the code base; `public/config.json` only
carries placeholders, so a local preview needs a real `config.json` (copy the deployed one) and a real Cognito user.
What is still fetchable without a session is the compiled static shell (HTML, JS, CSS, `config.json` with the public PKCE
client id and pool identifiers) — it contains no data and no fixtures; all research data comes from the AgentCore runtime,
which requires the user's JWT.
