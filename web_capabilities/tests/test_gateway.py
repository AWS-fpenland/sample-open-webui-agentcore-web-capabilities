...
from web_capabilities.gateway import (
    ...
    GatewayServiceError, GatewaySearchClient, GatewayTimeoutError, PROTOCOL_VERSION,
)
from web_capabilities.search import SearchContractError, SearchResponse
...
def test_runtime_credentials_refreshed_for_every_signed_request(monkeypatch):
    ...
    monkeypatch.setattr("web_capabilities.gateway.boto3.Session", session_factory)
    server = Server()
...
def test_credential_failure_is_opaque_and_precedes_transport(monkeypatch):
    ...
    monkeypatch.setattr("web_capabilities.gateway.boto3.Session", failed_session)
    server = Server()
...
def test_service_api_returns_parsed_search_response():
    server = Server()
    gateway = GatewaySearchClient("us-east-1", URL, "search___WebSearch",
                                  credentials=CREDS, transport=httpx.MockTransport(server))
    result = asyncio.run(gateway.search("query", max_results=2))
    assert isinstance(result, SearchResponse)
    assert server.requests[-1][1]["params"]["name"] == "search___WebSearch"


@pytest.mark.parametrize("tool_name", ["WebSearch", "search___Other", "___WebSearch"])
def test_service_api_rejects_unqualified_or_wrong_tool_name(tool_name):
    with pytest.raises(GatewayProtocolError):
        GatewaySearchClient("us-east-1", URL, tool_name, credentials=CREDS)
...
def test_references_rejected_before_any_schema_resolution(monkeypatch, keyword, location):
    ...
    for method in ("check_schema", "validate"):
        monkeypatch.setattr(f"web_capabilities.gateway.Draft202012Validator.{method}",
